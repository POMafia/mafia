var theme, minorErrors, fatalErrors, noMinor, noFatal, 
    possibleNightActions = ["kill", "protect", "inspect", "distract", "poison", "safeguard", "stalk", "watch", "convert", "massconvert", "curse", "copy", "detox", "dispel", "shield", "dummy"];
    ...
    for (r in this.roles) {
        ...
        if ("actions" in role) {
            ...
            for (var e in actions) {
                if (possibleNightActions.indexOf(e) !== -1 || /^dummy(?:\d+)?$/.test(e)) {
                    a = actions[e];
                    var mode = actions[a];
                    if ("mode" in mode) {
